    ...
    <div className="fixed inset-0 bg-black/70 flex items-center justify-center z-50 p-4">
      <div className="bg-gray-900 rounded-2xl p-6 w-full max-w-md">
        <div className="flex justify-between items-center mb-4">
          <div>
            <h2 className="text-xl font-bold text-white">Edit {show.name}</h2>
            <p className="text-xs text-gray-300 mt-1">Add your personal notes and rating</p>
          </div>
        ...
        <form onSubmit={handleSubmit} className="space-y-4">
          ...
          <div>
            <label className="flex items-center space-x-2 text-sm mb-2 text-white">
          ...
          <div>
            ...
            <div className="flex items-center gap-3">
              ...
              <input
                ...
                min="0"
                max="10"
                className="w-20 px-3 py-2 bg-gray-800 border border-gray-700 rounded-lg text-center text-white focus:outline-none focus:border-blue-500"
              />
            </div>
          </div>
          ...
          {(show?.personalRating || show?.personalNotes || show?.status) && (
            <div className="bg-gray-800/50 rounded-lg p-3 text-sm">
              <p className="text-xs text-gray-300 mb-2">Currently Saved:</p>
              {show?.status && <p className="text-xs text-white flex items-center gap-1"><Pin className="w-3 h-3" /> Status: {show.status}</p>}
              {show?.personalRating && <p className="text-white flex items-center gap-1"><Star className="w-3 h-3" /> Rating: {show.personalRating}/10</p>}
              {show?.personalNotes && <p className="text-xs text-gray-300 mt-1 line-clamp-2 italic"> {show.personalNotes}</p>}
            </div>
          )}
          ...
          <div className="flex space-x-3 pt-2">
            ...
            <button type="button" onClick={onClose} className="flex-1 py-2 bg-gray-700 text-white rounded-lg font-semibold hover:bg-gray-600">
              Cancel
            </button>
          </div>
        </form>
